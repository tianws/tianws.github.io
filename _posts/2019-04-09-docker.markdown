...
layout:     post
title:      "Docker的安装和配置"
subtitle:   "Docker和Nvidia-Docker的安装"
...
1. 转至　[`https://download.docker.com/linux/ubuntu/dists/`](https://download.docker.com/linux/ubuntu/dists/) ，选择您的 Ubuntu 版本（`lsb_release -cs`c查看），浏览至 `pool/stable/`，然后选择 `amd64`, `armhf`, `arm64`, `ppc64el`或者 `s390x`。下载适用于您想要安装的`.deb`文件。
