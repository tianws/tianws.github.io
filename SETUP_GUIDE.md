...
#### 2. 图片使用流程
为了在保证图片质量的同时，最大化网站性能，项目采用源码与生成资源分离的工作流。

1.  **存放原图**: 将所有未经压缩的高质量原图，存放到 `_source_images/` 目录中，并保持您希望的子目录结构。这个目录是您图片资产的永久备份。
2.  **文章中引用**: 在 Markdown 文件中，正常引用 `img/` 目录下的路径，例如 `![alt](/img/some-folder/image.png)`。
3.  **生成压缩图片**: 在发布或预览前，运行以下命令：
    ```bash
    npm run optimize:img
    ```
    该命令会自动读取 `_source_images/` 下的所有图片，将其压缩后，按原目录结构输出到 `img/` 目录中。`img/` 目录不纳入版本库管理。
